...
<!--            <span>&nbsp;{{ article.username }}</span>-->
      ...
      <div v-if="!$common.isEmpty(recommendArticles)"
           style="padding: 25px;border-radius: 10px;margin-top: 0px;animation: hideToShow 1s ease-in-out"
        ...
        <div v-for="(article, index) in recommendArticles"
             :key="index"
             @click="openLoadUrl(article.id)" style="display: inline-block;width: 50%">
...
<!--          </div>-->
      ...
      <div style="background: var(--background);">
        ...
        <div class="article-container my-animation-slide-bottom">
          <blockquote>
            ...
            <div  style="text-align: center;color: blue;font-size: 18px">
              你不看<span style="color:red;font-size:26px">还有其他兄弟要看</span>不要断了大家的后路
            </div>
          </blockquote>
        ...
        <div id="toc" class="toc" @click="openQQWindows"></div>
      </div>
...
<!--      </div>-->
    ...
    <el-dialog title="密码解锁"
               ...
               center>
      <div>
        <div class="tocd" @click="openQQWindows"></div>
        <div v-if="articleContentPre" style="text-align: center">这里是预览图展示区域 请滑动屏幕到下方输入密码</div>
        <div v-html="articleContentPre" class="entry-content"></div>
<!--        <div style="text-align: center">-->
<!--          <div>扫描二维码点击底部 <span style="color: red">获取积分</span> 看广告</div>-->
<!--          <div v-if="id">-->
<!--            <el-image  style="width: 100px;height: 100px" :src="getImages()"></el-image>-->
<!--          </div>-->
<!--        </div>-->
<!--        <div style="text-align: center">-->
<!--          <div class="password-content">请完整看完广告,才可以获取到密码</div>-->
<!--        </div>-->
        <div style="margin-top: 10px">
          <blockquote>
            <div style="text-align: center" @click="openHelpUrl()">
              点我淘宝省钱购物
            </div>
            <div id="nativeShare"  class="collection-btn" @click="openUrlQQ('wx')" style="text-align: center;margin-top: 5px">
              点我观看小程序广告获取密码
            </div >
            <div id="nativeShare"  class="collection-btn" @click="openUrlQQ('qq')" style="text-align: center;margin-top: 5px" v-if="Object.keys(likeData).includes('qq_like')&& likeData['qq_like'].length>0">
              点我分享至QQ群获取密码
            </div >
            <div style="text-align: center" v-if="Object.keys(likeData).includes('qq_like')&& likeData['qq_like'].length>0">QQ里打开点击右上角分享,手机浏览器打开点击上方QQ分享</div>
  ...
  export default {
    ...
    methods: {
      ...
      getRecommendArticles() {
        this.$http.post(this.$constant.baseURL + "/article/listArticleRecommend", {
          labelId: this.id
        })
          .then((res) => {
      ...
      getImages(){
        return window['qrImageUrl']+this.id+'.png';
      },
      openUrlQQ(name){
        location.href=window['txCodeUrl']+name+'.html?text='+this.likeData['qq_like']+'&id='+this.id
      },
      openHelpUrl(){
        location.href='https://qxxzj.yhzu.cn'
        // location.href =window['qrCodeUrl']+"8"
        // location.reload()
      },
      openLoadUrl(id){
        console.log(id)
        location.href =window['qrCodeUrl']+id
        location.reload()
      },
      openQQWindows(){
        window.open(window['chiguaHref'])
      },
      clickTocButton() {
        let display = $(".toc");
        ...
        this.$http.get(this.$constant.baseURL + "/article/getArticleById", {id: this.id, password: password})
          ...
          .catch((error) => {
            if ("密码错误" === error.message.substr(0, 4)) {
              ...
              console.log(error)
              if (error.data){
               this.likeData = JSON.parse(error.data.link)
                const md = new MarkdownIt({breaks: true}).use(require('markdown-it-multimd-table'));
                this.articleContentPre = md.render(error.data.article_pre);
              }
