<template>
  ...
  <div v-else>
    <div v-if="getFileExtension(fileName,['png','jpg','jpeg'])">
      <div style="width: 150px; height: 150px">
        <el-image
          style="width: 140px;height: 130px"
          :src="fileName"
          :preview-src-list="[fileName]">
        </el-image>
        <div style="text-align: center;cursor: pointer;color: black" @click="closeFile">删除</div>
      </div>

    </div>
    <div style="width: 300px;" v-if="getFileExtension(fileName,['mp4'])">
      <videoPlayer :url="{src: fileName}" ></videoPlayer>
      <div style="text-align: center;cursor: pointer;color: black" @click="closeFile">删除</div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getFileExtension(fileName,arrays) {
        let fileSubName = fileName.slice(fileName.lastIndexOf(".") + 1);
        return arrays.includes(fileSubName)
      },
