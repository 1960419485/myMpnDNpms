<template>
  <div>
    ...
    <el-form :model="article" :rules="rules" ref="ruleForm" label-width="150px"
      ...
      <el-form-item label="预览" prop="articlePre">
        <mavon-editor ref="mds" @imgAdd="imgAddMd2" v-model="article.articlePre"/>
      </el-form-item>
...
<!--        <el-input maxlength="1000" v-model="article.videoUrl"></el-input>-->
        <uploadPicture :file-name="article.videoUrl" accept="video/*" :isAdmin="true" list-type="file" prefix="videoPlay" style="margin-top: 10px" @addPicture="addVideoUrl"
                       :maxSize="200"
      ...
      <el-form-item v-if="article.viewStatus === false" label="不可见时的访问密码" prop="password">
        <el-input style="width: 400px" maxlength="30" v-model="article.password">
          <template #append>
            <div style="cursor: pointer" @click="article.password = ('000000' + Math.floor(Math.random() * 999999)).slice(-6);">随机密码</div>
          </template>
        </el-input>
      </el-form-item>
      ...
      <el-form-item label="封面" prop="articleCover">
        ...
        <uploadPicture  :file-name="article.articleCover" :isAdmin="true" :prefix="'articleCover'" style="margin-top: 10px" @addPicture="addArticleCover"
                       :maxSize="5"
                       :maxNumber="1"></uploadPicture>
      </el-form-item>
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.$common.isEmpty(this.id)) {
              ...
              this.article.tips= '微信小程序搜索壁纸:  {'+this.article.articleTitle +" }   观看广告获取密码"
              let addArticleData = JSON.parse(JSON.stringify(this.article))
              addArticleData.dappUrl = JSON.stringify(this.article.dappUrl)
              this.saveArticle(addArticleData, "/article/saveArticle")
            } else {
              this.article.id = this.id;
              this.article.tips= '微信小程序搜索壁纸:  {'+this.article.articleTitle +" }   观看广告获取密码"
              let addArticleData = JSON.parse(JSON.stringify(this.article))
              addArticleData.dappUrl = JSON.stringify(this.article.dappUrl)
              this.saveArticle(addArticleData, "/article/updateArticle")
            }
          } else {
            this.$message({
              message: "请完善必填项！",
