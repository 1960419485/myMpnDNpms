...
<!--          <el-switch @click.native="changeStatus(scope.row, 1)" v-model="scope.row.viewStatus"></el-switch>-->
      ...
      <el-table-column label="二维码" align="center" width="150px">
        <template slot-scope="scope">
          <vue-qr
            style="width: 130px;height: 130px"
            :text="getQrCodeUrl(scope.row.id)"
            :callback="Qrtext"
            :qid="scope.row.id"
      ...
      <el-table-column label="操作" fixed="right" width="240" align="center">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.dappId" @click="openLikeInfo(scope.row)" type="text" icon="el-icon-edit"  >
            小程序链接
          </el-button>
    ...
    <el-dialog title="修改访问链接" :visible.sync="createLikeStatus" width="680px" center  :close-on-click-modal="false">
      <div>
        <el-form ref="form" :model="updateLinkFrom"  label-width="80px">

          <el-form-item label="复制链接" prop="wx_like">
            <el-input v-model="updateLinkFrom.dappId"  >
              <template #append>
                <div style="cursor: pointer" @click="copyText">点我复制</div>
              </template>
            </el-input>
          ...
          <el-row :gutter="30">
            <el-col :span="12">
              <el-form-item label="微信分享链接" prop="wx_like">
                <el-input v-model="updateLinkFrom.likeData.wx_like" placeholder="请输入微信小程序链接"/>
              </el-form-item>
            </el-col>

            <el-col :span="12">
              <el-form-item label="QQ分享链接" prop="qq_like">
                <el-input v-model="updateLinkFrom.likeData.qq_like" placeholder="请输入QQ分享"/>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="createLikeStatus=false">取 消</el-button>
    ...
    methods: {
      updateArticle() {
        this.$confirm('确认保存？', '提示', {
          ...
          this.updateLinkFrom.likeData=JSON.stringify(this.updateLinkFrom.likeData)
          this.$http.post(this.$constant.baseURL + '/admin/article/updateLike', this.updateLinkFrom, true)
            .then((res) => {
              this.$message({
                message: "保存成功！",
                type: "success"
              });
              this.createLikeStatus = false;
             this.getArticles();
            })
            .catch((error) => {
              this.$message({
      ...
      copyText(){
        ...
        textCopy.value = this.updateLinkFrom.dappId
      ...
      openLikeInfo(item){
        if (item.dappUrl){
          this.updateLinkFrom.id = item.id;
          this.updateLinkFrom.dappId= 'id='+item.dappId+'&index=0';
          this.updateLinkFrom.likeData=JSON.parse(item.dappUrl)
          this.createLikeStatus = true
        }else {
          this.$message({
            type: 'success',
            message: '没有链接数据!'
